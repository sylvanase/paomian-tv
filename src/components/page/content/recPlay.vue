<template>
    <section>
        ...
        <el-table v-loading="tableLoading" class="table-expand" :data="tableList" stripe border :max-height="tableHeight"
            ...
            <el-table-column prop="playName" label="剧本名称"></el-table-column>
            <el-table-column prop="movieNames" label="相关电影">
                <template scope="scope">
                    {{ scope.row.movieNames.join(' , ') }}
                </template>
            </el-table-column>
            <el-table-column prop="playAttrs" label="剧本属性">
                <template scope="scope">
                    {{ scope.row.playAttrs.join(' , ') }}
                </template>
            </el-table-column>
        ...
        methods: {
            ...
            fetchList() { //获取列表
                ...
                if (isNaN(_self.filters.kw)) { //输入不为数字，值传入kw
                    if (_self.filters.type == 0) { // 0为按剧本搜索
                        paras.append('playName', _self.filters.kw);
                    } else {
                ...
                httpPost('contentRecPlayList', paras, _self, function (res) {
