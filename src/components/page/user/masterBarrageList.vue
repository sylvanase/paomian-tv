        ...
        <el-table v-loading="tableLoading" :data="tableList" stripe border style="width: 100%;">
            ...
            <el-table-column prop="coverUrl" label="封面(点击播放)" width="135">
                <template scope="scope">
                    <img @click="playVideo(scope.row.simpleVideoPo)" v-if="scope.row.simpleVideoPo.coverUrl !== ''" style="width: 100%;cursor: pointer;" :src="scope.row.simpleVideoPo.coverUrl" alt="视频封面"/>
                    <span @click="playVideo(scope.row.simpleVideoPo)" v-else>封面为空</span>
                </template>
            </el-table-column>
            <el-table-column prop="text" min-width="150" label="弹幕内容"></el-table-column>
    ...
    export default {
        ...
        computed: {
            detail(){
                ...
                this.tableList = [];
                this.title = "小号弹幕列表，主账号id：'" + this.masterData.uid + '，昵称：' + this.masterData.userName;
                if (this.visible) {
                    this.fetchList();
                }
            }
        ...
        methods: {
            ...
            fetchList() {    //获取列表
                ...
                httpGet('userMasterBarrage', paras, _self, function (res) {
