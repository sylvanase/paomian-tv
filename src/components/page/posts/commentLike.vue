<template>
    <el-dialog title="评论点赞" :value="value" v-model="visible" @close="resetFormData" size="tiny">
        <el-form :model="likeData" label-width="80px" :rules="likeRules" ref="likeData"
            ...
            <el-form-item label="数量" prop="num" style="margin-bottom: -20px;">
                <el-input-number placeholder="单次限制最多100个" step="1" :min="0" :max="100"
    ...
    export default {
        ...
        computed: {
            detail() { //返回详情
                ...
                _self.likeData.id = _self.commentId; // 需要点赞的评论id
            }
        },
        methods: {
            change() {
            ...
            formSubmit() { //提交表单
                let _self = this;
                let paras = {
                    id: _self.likeData.id,
                    count: _self.likeData.num
                };
                httpGet('commentLikeAdd', paras, _self, function (res) {
                    try {
